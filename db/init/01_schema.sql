...
CREATE TABLE raw_events (
    ...
    -- full parsed message: body/attributes/scope/resource
...
CREATE INDEX idx_fact_user_prompts_event_timestamp ON fact_user_prompts (
    event_timestamp
);
...
CREATE INDEX idx_fact_api_requests_event_timestamp ON fact_api_requests (
    event_timestamp
);
...
CREATE TABLE fact_tool_events (
    id TEXT
    PRIMARY KEY,          -- decision event id, falling back to result event id
    session_id TEXT REFERENCES dim_sessions (session_id),
    user_email TEXT REFERENCES dim_employees (email),
...
CREATE INDEX idx_fact_tool_events_decision_timestamp ON fact_tool_events (
    decision_timestamp
);
...
CREATE INDEX idx_fact_api_errors_event_timestamp ON fact_api_errors (
    event_timestamp
);
